...
def from_dataframe(
    ...
    *,
    ...
    """Convert a DataFrame back to a PM4Py EventLog, adding the space extension if needed."""
    log = pm4py.convert.convert_to_event_log(df, case_id_key=case_id_key)
    
    # Inject space extension if spatial columns are present
    if any(col.startswith("space:") for col in df.columns):
        if "Space" not in log.extensions:
            log.extensions["Space"] = {"prefix": "space", "uri": "space.xesext"}
