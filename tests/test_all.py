"""End-to-end tests for pm4py.spatial after deep-research fixes."""
...
def test_non_mutating_normalize_types():
    ...
    result = pm4py.spatial.normalize_types(df)
    assert df["space:x"].dtype in (object, "string"), "Input should still be strings"
...
def test_non_mutating_normalize_crs():
    ...
    result = pm4py.spatial.normalize_crs(df, "EPSG:32618", log_crs="EPSG:4326")
...
def test_local_meters_transform_error():
    ...
    try:
        pm4py.spatial.normalize_crs(df, "EPSG:4326", log_crs="LOCAL_METERS")
...
def test_nan_inf_detection():
    ...
    for val, label in [(float("inf"), "inf"), (float("-inf"), "-inf"), (float("nan"), "NaN")]:
        ...
        try:
            pm4py.spatial.validate(df, log_crs="EPSG:4326")
            print(f"  FAIL: {label} should have raised")
            return False
        except pm4py.spatial.SpatialValidationError:
...
def test_strict_mode_range_check():
    ...
    try:
        pm4py.spatial.validate(df, log_crs="EPSG:4326", strict=True)
    ...
    pm4py.spatial.validate(df_ok, log_crs="EPSG:4326", strict=True)
...
def test_strip_spatial():
    ...
    result = pm4py.spatial.strip_spatial(df)
...
def test_is_transformable():
    ...
    assert pm4py.spatial.is_transformable("EPSG:4326") is True
    assert pm4py.spatial.is_transformable("EPSG:32618") is True
    assert pm4py.spatial.is_transformable("LOCAL_METERS") is False
    assert pm4py.spatial.is_transformable("LOCAL_FEET") is False
    assert pm4py.spatial.is_transformable("LOCAL") is False
    assert pm4py.spatial.is_transformable("PIXEL") is False
    print("  PASS")
